...
if __name__ == "__main__":
  ...
  #     numActiveColumnsPerInhArea=1, # Number features active at a time
      localAreaDensity=0.05, # percent of columns active at a time
    ...
    sp=SpatialPooler(
      inputDimensions=(32,32), # Size of image patch
      columnDimensions=(16,16), # Number of potential features
      potentialRadius=10000, # Ensures 100% potential pool
      potentialPct=1, # Neurons can connect to 100% of input
      globalInhibition=True,
#       localAreaDensity=-1, # Using numActiveColumnsPerInhArea
      localAreaDensity=0.05, # one percent of columns active at a time
      #numActiveColumnsPerInhArea=-1, # Using percentage instead
#       numActiveColumnsPerInhArea=1, # Only one feature active at a time
  ...
  accuracy = tb.test(testingVectors, testingTags, clf, verbose=0, learn=True)
